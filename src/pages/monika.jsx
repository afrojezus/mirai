    ...
    next = () => {
        ...
        setTimeout(() => {
            if (this.state.topic + 1 > Object.values(moni).length - 1) {
                ...
                this.setState({dialog: moni[this.state.topic + 1][0], text: 0, topic: this.state.topic + 1, wait: false})
            }
        }, 10000) 
        }
        else
    ...
    render () {
        ...
        return (
            <div className={classes.root}>
                <img src={img ? monispace : scare ? moniscare : spaceBgEmpty} alt='' style={scare ? {transform: 'scale(3) translateY(20%)', animation: 'none'} : null} className={classes.bgImage} />
                <M.Card className={classes.dialogue} style={wait ? {opacity: 0, pointerEvents: 'none'} : null}>
                <M.CardContent>
                <M.Typography type='body1' className={classes.text}>{dialog}</M.Typography>
                </M.CardContent>
